...
from src.model import build_layout_model, train_model
...
def main():
    ...
    print(f"Generated {data['n_samples']} documents, classes: {data['class_names']}")
    ...
    print(f"Model: DeepLabV3 ResNet-50 ({sum(p.numel() for p in model.parameters()):,} params)")

    model, history = train_model(model, train_loader, val_loader, epochs=a.epochs, lr=a.lr, device=a.device, num_classes=NUM_CLASSES)
